...
public class TicketServiceImpl extends TransactionalService implements TicketService {
    ...
    @Override
    public Page<TicketEntity> search(TicketCriteria criteria, Pageable pageable) {
        try {
            ...
            List<TicketEntity> entities = tickets.getContent()
                    .stream()
                    .map(t -> {
                        final Ticket ticket = new Ticket(t);

                        //Retrieve application name
                        if (StringUtils.isNotEmpty(t.getApplication())) {
                            ApplicationEntity application = applicationService.findById(t.getApplication());
                            ticket.setApplication(application.getName());
                        }

                        //Retrieve api name
                        if (StringUtils.isNotEmpty(t.getApi())) {
                            ApiEntity api = apiService.findById(t.getApi());
